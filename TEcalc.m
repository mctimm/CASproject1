...
function storeTE(eps, mpop, D)
    ...
    for i = 1:3
        maxTD = 0;
        maxBU = 0;
        for k = 1:4 %running out a memory, will have to find a work around
...
function result = calcTE(v1, v2, k)
    ...
    calc = javaObject('infodynamics.measures.discrete.TransferEntropyCalculatorDiscrete', 33, k, 1, 1, 1, 1);
    ...
    result1 = calc.computeAverageLocalOfObservations();
    clear calc;
    result = result1;
